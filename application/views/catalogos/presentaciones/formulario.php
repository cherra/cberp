<?php echo form_open($action, array('class' => 'form-horizontal', 'name' => 'form', 'id' => 'form', 'role' => 'form')) ?>
    ...
    <?php if(isset($subproductos)){ ?>
    <div class="form-group">
        <label class="col-sm-3" for="id_producto">Subproducto</label>
        <div class="col-sm-8 col-md-6 col-lg-4">
            <select name="id_subproducto" class="form-control required">
                <option value="">Selecciona un subproducto...</option>
                <?php foreach($subproductos as $subproducto){ ?>
                <option value="<?php echo $subproducto->id_subproducto; ?>" <?php if(isset($datos) && $datos->id_subproducto == $subproducto->id_subproducto) echo "selected"; ?>><?php echo $subproducto->nombre; ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
